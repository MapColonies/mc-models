...
const OutputFolder = "./src/types";
const ServiceUrl = "http://mc-models";
...
const getUrl = (fullPath) => {
   const relativePath = path.normalize(path.relative(process.cwd(), fullPath)).replace(/\\/g, "/");
   if (relativePath.charAt(0) == ".") return ServiceUrl + relativePath.slice(1);
   else return ServiceUrl + "/" + relativePath;
};

const resolver = {
   order: 1,
   canRead: /^http:\/\/mc-models/i,
   read(file) {
      const path = file.url.replace("http://mc-models", ".");
      return new Promise((resolve, reject) => {
         if (path.toLowerCase().startsWith("./Schema/geojson/".toLowerCase())) return resolve("{}");
...
const compileAndMap = async (fullPath) => {
   if (!fullPath.endsWith(".base.json")) return Promise.resolve();
   ...
   return new Promise((resolve, reject) => {
      json2ts
         ...
         .then((ts) => {
            ...
            index.push(`export * from './${path.basename(fullPath, path.extname(fullPath))}';`);
            resolve();
         });
   });
};

...
@catalogDBEntity({
  table: 'records',
  className: 'QuantizedMeshBestEntity',
})
@graphqlClass({ alias: 'QuantizedMeshBestRecord' })
export class PycswQuantizedMeshBestCatalogRecord extends QuantizedMeshBestMetadata implements IPycswCoreModel, IOrmCatalog {
  //#region CORE: id
  @pycsw({
  ...
  //#endregion
  public typeName = 'mc_MCQuantizedMeshBestRecord';
  ...
  //#endregion
  public schema: string | undefined = 'mc_quantized_mesh_best';
  ...
  public static getPyCSWMappings(): IPropPYCSWMapping[] {
    const ret = [];
    const layer = new PycswQuantizedMeshBestCatalogRecord();
    for (const prop in layer) {
      const pycswMap = getPyCSWMapping<PycswQuantizedMeshBestCatalogRecord>(layer, prop);
      if (pycswMap) {
        ret.push({
          prop: prop,
          ...pycswMap,
  ...
  public static getFieldConfigs(): IPropFieldConfigInfo[] {
    ...
    const layer = new PycswQuantizedMeshBestCatalogRecord();
    for (const prop in layer) {
      const fieldConfigMap = getFieldConfig<PycswQuantizedMeshBestCatalogRecord>(layer, prop);
      if (fieldConfigMap) {
  ...
  public getORMCatalogEntityMappings(): ICatalogDBEntityMapping {
    return getCatalogDBEntityMapping(PycswQuantizedMeshBestCatalogRecord);
  }
}

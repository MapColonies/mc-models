...
@catalogDBEntity({
  table: 'records',
  className: 'BestEntity',
})
@graphqlClass({ alias: 'BestRecord' })
export class PycswBestCatalogRecord extends BestMetadata implements IPycswCoreModel, IOrmCatalog {
  //#region CORE: id
  @pycsw({
    profile: 'mc_raster',
    xmlElement: 'mc:id',
  ...
  //#endregion
  public typeName = 'mc_MCBestRecord';
  ...
  public static getPyCSWMappings(): IPropPYCSWMapping[] {
    const ret = [];
    const layer = new PycswBestCatalogRecord();
    for (const prop in layer) {
      const pycswMap = getPyCSWMapping<PycswBestCatalogRecord>(layer, prop);
      if (pycswMap) {
        ret.push({
          prop: prop,
          ...pycswMap,
  ...
  public static getFieldConfigs(): IPropFieldConfigInfo[] {
    ...
    const layer = new PycswBestCatalogRecord();
    for (const prop in layer) {
      const fieldConfigMap = getFieldConfig<PycswBestCatalogRecord>(layer, prop);
      if (fieldConfigMap) {
  ...
  public getORMCatalogEntityMappings(): ICatalogDBEntityMapping {
    return getCatalogDBEntityMapping(PycswBestCatalogRecord);
  }
}

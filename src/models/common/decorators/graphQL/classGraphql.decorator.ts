...
import { getFieldConfig } from '../fieldConfig/fieldConfig.decorator';
...
export function getGraphQLMappings(object: KeyValueDict, fields?: string[]): IPropGraphQLMapping[] {
  ...
  props.forEach((prop) => {
    ...
    const fieldConfig = getFieldConfig(object, prop);
    if (graphQLMap && tsTypesMap) {
      ret.push({
        prop: prop,
        ...graphQLMap,
        ...tsTypesMap,
        isAutoGenerated: fieldConfig?.isAutoGenerated,
        isLifecycleEnvolved: fieldConfig?.isLifecycleEnvolved,
      });
    }
  });
  return ret;

...
export enum ValidationStatus { valid, invalid, missingValidator };
...
export class SchemaValidator{
    ...
    private async getValidator (schemaUrl: string) {
        if(!this.validators[schemaUrl] && this.tryLoadMissingSchema) 
            await this.LoadVaidator(schemaUrl);
        if (this.validators[schemaUrl] ){
            const validate = await this.validators[schemaUrl].validate;
            if(!this.validators[schemaUrl].error)
                return validate;
            else           
                throw this.validators[schemaUrl].error;
        }
        return null;
    }
    ...
    async LoadVaidator (schemaUrl: string) : Promise<void> {
        ...
        try {
            ...
            this.validators[schemaUrl].validate = this.ajv.compileAsync(schema)
                .then((validator: Ajv.ValidateFunction) => validator, (err:Error) => {
    ...
    async validate(schemaUrl: string,data:unknown):Promise<ValidationResult>
    {
        ...
        if(!validate)
        return {status:ValidationStatus.missingValidator};
